...
#[inline(always)]
pub unsafe fn tlb_insert_read(cpu: &mut Cpu, vaddr: u64, ram: *mut u8, phys: u64) {
    let page = vaddr & !PAGE_MASK;
...
#[inline(always)]
pub unsafe fn tlb_insert_write(cpu: &mut Cpu, vaddr: u64, ram: *mut u8, phys: u64) {
    let page = vaddr & !PAGE_MASK;
    let phys_page = phys & !PAGE_MASK;
    let set_idx = ((vaddr >> PAGE_SHIFT) as usize) & (TLB_SETS - 1);
...
#[inline(always)]
pub unsafe fn tlb_insert_code(cpu: &mut Cpu, vaddr: u64, ram: *mut u8, phys: u64) {
    let page = vaddr & !PAGE_MASK;
    let phys_page = phys & !PAGE_MASK;
    let set_idx = ((vaddr >> PAGE_SHIFT) as usize) & (TLB_SETS - 1);
...
pub unsafe fn walk_page_tables(
    ...
    if pml4_addr + 8 > ram_size as u64 {
        return Err(MemFault::PageFault { vaddr, error_code: pf_error(write, cpu.cpl == 3, false) });
    }
    let pml4e = read_unaligned((ram as usize + pml4_addr as usize) as *const u64);
    if pml4e & PTE_PRESENT == 0 {
        return Err(MemFault::PageFault { vaddr, error_code: pf_error(write, cpu.cpl == 3, false) });
    }
    check_perms(pml4e, write, exec, cpu.cpl, cpu.cr0, cpu.cr4, cpu.efer)?;
    ...
    check_perms(pdpe, write, exec, cpu.cpl, cpu.cr0, cpu.cr4, cpu.efer)?;
    ...
    check_perms(pde, write, exec, cpu.cpl, cpu.cr0, cpu.cr4, cpu.efer)?;
    ...
    check_perms(pte, write, exec, cpu.cpl, cpu.cr0, cpu.cr4, cpu.efer)?;
...
unsafe fn check_perms(
    ...
    if write && (pte & PTE_WRITE == 0) {
        ...
        if cpl == 3 || (cr0 & CR0_WP != 0) {
            return Err(MemFault::PageFault { vaddr: 0, error_code: 0 });
        }
    }
    // User check
    if cpl == 3 && (pte & PTE_USER == 0) {
        return Err(MemFault::PageFault { vaddr: 0, error_code: 0 });
    }
    // NX check
...
#[inline(always)]
fn pf_error(write: bool, user: bool, present: bool) -> u32 {
    let mut code = 0u32;
    if present { code |= 1; }
    if write { code |= 2; }
    if user { code |= 4; }
...
pub unsafe fn load_u8(cpu: &mut Cpu, ram: *mut u8, ram_size: u32, vaddr: u64) -> Result<u8, MemFault> {
    ...
    tlb_insert_read(cpu, vaddr, ram, phys);
...
pub unsafe fn store_u8(cpu: &mut Cpu, ram: *mut u8, ram_size: u32, vaddr: u64, val: u8) -> Result<(), MemFault> {
    ...
    tlb_insert_write(cpu, vaddr, ram, phys);
    if phys < ram_size as u64 {
...
pub unsafe fn fetch_u8(cpu: &mut Cpu, ram: *mut u8, ram_size: u32, vaddr: u64) -> Result<u8, MemFault> {
    ...
    tlb_insert_code(cpu, vaddr, ram, phys);
    if phys < ram_size as u64 {
        Ok(read_unaligned((ram as usize + phys as usize) as *const u8))
    } else {
        Ok(0xFF)
