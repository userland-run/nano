...
async function runWasm(kernel, opts) {
  ...
  const exit = await new Promise((resolve) => {
    ...
    worker.onMessage((m) => { if (m?.type === "exit") finish({ exitCode: m.code ?? 0 }); else if (m?.type === "fatal") finish({ exitCode: 1, error: m.error }); });
  ...
  return { exitCode: exit.exitCode, stdout: outBuf + dec.decode(), stderr: errBuf, error: exit.error, pid: proc.pid };
}
...
export { runWasm, scopesToPreopens };
